...
###############################################
# And here we create the training data
#
# Varnames of Importance:
# tupleArray: the array of training glyphs

#Goes through the .data file and pushes each glyph into tuples
filename = sys.argv[1]
...
# decoded result.
# 
# If we want to split training from testing.... Vars TensorFlow Needs:
...
with tf.Session() as sess:
  ...
  for iter in range(0,1):
    ...
    testlabels_arr = []
    testline = np.zeros(shape=(1, 27))

    #Fun parsy times inside this loop
    #for line in file_test: #t3.py
      ...
      if(line!="\n"):		#Avoid printing empty lines
        ...
        for place in range(0, len(line)):
          if(line[place]=="," and colcount<27):
            testline[0][colcount]=line[(place-8):place]
            colcount+=1
          #elif(colcount==27):
            #if(line[place]!="\n" and line[place]!=" "):
        ...
        prediction = model.predict(testline)	#Testing line by line
        #print("Pred:", predictions)
        #print("Actual: ", sorted_labels_arr[np.argmax(prediction)])
        predicted_char = sorted_labels_arr[np.argmax(prediction)]
        sys.stdout.write(predicted_char)
        #print(sorted_labels_arr[np.argmax(prediction)])		#Outputs predicted valye
